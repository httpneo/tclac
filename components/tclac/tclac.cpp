...
ClimateTraits tclacClimate::traits() {
	...
	// Ich erkläre hiermit verantwortungsvoll, dass ich alles von christoph5180 genommen habe
...
void tclacClimate::loop()  {
	// Wenn etwas im UART-Puffer vorhanden ist, dann lesen wir es
	if (esphome::uart::UARTDevice::available() > 0) {
		dataShow(0, true);
		dataRX[0] = esphome::uart::UARTDevice::read();
		// Wenn das empfangene Byte nicht der Header (0xBB) ist, dann beenden wir die Schleife
		if (dataRX[0] != 0xBB) {
			ESP_LOGD("TCL", "Wrong byte");
			dataShow(0,0);
			return;
		}
		// Wenn der Header (0xBB) übereinstimmt, dann beginnen wir mit dem Lesen von 4 weiteren Bytes hintereinander
		// Manchmal muss man bei einigen Klimaanlagen eine Verzögerung(5) zwischen den Paketen hinzufügen. Warum - weiß ich nicht, aber das ist notwendig. Aber nicht immer. Manchmal - ja. Aber nicht jedes Mal. Selten. Es kommt vor.
		// delay(5);
		...
		// Aus den ersten 5 Bytes brauchen wir den fünften - er enthält die Länge der Nachricht
		...
		this->dataShow(0,0);
		// Nachdem wir alles aus dem Puffer gelesen haben, beginnen wir mit der Verarbeitung der Daten
		this->readData();
	}
}
...
void tclacClimate::readData() {
	...
	if (dataRX[MODE_POS] & ( 1 << 4)) {
		// Wenn die Klimaanlage eingeschaltet ist, dann analysieren wir die Daten zur Anzeige
		...
		// Verarbeitung von Voreinstellungsdaten
		...
		// Wenn die Klimaanlage ausgeschaltet ist, dann werden alle Modi als ausgeschaltet angezeigt
...
void tclacClimate::control(const climate::ClimateCall &call) {
	...
	// Dies und was folgt habe ich von Vi3jo gekürzt.
...
void tclacClimate::takeControl() {
	...
	// Wir schalten den Summer ein oder aus, je nach Schalter in den Einstellungen
	if (beeper_status_){
	...
	// Wir schalten das Display der Klimaanlage je nach Schalter in den Einstellungen ein oder aus
	// Wir schalten das Display nur ein, wenn die Klimaanlage in einem der Betriebsmodi läuft
	
	// WARNUNG! Wenn das Display ausgeschaltet wird, schaltet die Klimaanlage automatisch in den automatischen Modus!
	...
	// Wir konfigurieren den Betriebsmodus der Klimaanlage
	switch (this->mode) {
	...
	// Wir konfigurieren den Lüftermodus
	...
	// Wir stellen den Schwungmodus der Klappen ein
	...
	// Wir stellen die Voreinstellungen der Klimaanlage ein
	if (this->preset.has_value()) {
        ...
        //Modus der Klappen
		//	Vertikale Klappe
		//		Schwung der vertikalen Klappe [10 Byte, Maske 00111000]:
		//			000 - Schwung deaktiviert, Klappe in der letzten Position oder fixiert
		//			111 - Schwung in ausgewähltem Modus aktiviert
		//		Modus des Vertikalschwungs (der Fixierungsmodus spielt keine Rolle, wenn Schwung aktiviert ist) [32 Byte, Maske 00011000]:
		//			01 - Schwung von oben nach unten, STANDARD
		//			10 - Schwung in der oberen Hälfte
		//			11 - Schwung in der unteren Hälfte
		//		Fixierungsmodus der vertikalen Klappe (Schwungmodus spielt keine Rolle, wenn Schwung deaktiviert ist) [32 Byte, Maske 00000111]:
		...
		//			101 - Fixierung unten
		//	Horizontale Klappen
		//		Schwung der horizontalen Klappen [11 Byte, Maske 00001000]:
		//			0 - Schwung deaktiviert, Klappen in letzter Position oder fixiert
		//			1 - Schwung in ausgewähltem Modus aktiviert
		//		Modus des Horizontalschwungs (der Fixierungsmodus spielt keine Rolle, wenn Schwung aktiviert ist) [33 Byte, Maske 00111000]:
		//			001 - Schwung von links nach rechts, STANDARD
		//			010 - Schwung links
		//			011 - Schwung in der Mitte
		//			100 - Schwung rechts
		//		Fixierungsmodus der horizontalen Klappen (Schwungmodus spielt keine Rolle, wenn Schwung deaktiviert ist) [33 Byte, Maske 00000111]:
		//			000 - keine Fixierung, STANDARD
		//			001 - Fixierung links
		//			010 - Fixierung zwischen linker Seite und Mitte
		//			011 - Fixierung in der Mitte
		//			100 - Fixierung zwischen Mitte und rechter Seite
		//			101 - Fixierung rechts
		
		
	// Wir stellen den Modus für den Vertikalschwung ein
	...
	// Wir stellen den Modus für den Horizontalschwung ein
	...
	// Wir stellen die Fixierungsposition der vertikalen Klappe ein
	...
	// Wir stellen die Fixierungsposition der horizontalen Klappen ein
	...
	// Wir sammeln ein Byte-Array zum Senden an die Klimaanlage
	dataTX[0] = 0xBB;	//Start-Header-Byte
	dataTX[1] = 0x00;	//Start-Header-Byte
	dataTX[2] = 0x01;	//Start-Header-Byte
	dataTX[3] = 0x03;	//0x03 - Steuern, 0x04 - Abfrage
	dataTX[4] = 0x20;	//0x20 - Steuern, 0x19 - Abfrage
	dataTX[5] = 0x03;	//??
	dataTX[6] = 0x01;	//??
	...
	dataTX[30] = 0x00;	//??
	dataTX[31] = 0x00;	//??
	//dataTX[32] = 0x00;	//0,0,0,Modus Vertikalschwung(2),Fixierung Vertikal(3)
	//dataTX[33] = 0x00;	//0,0,Modus Horizontalschwung(3),Fixierung Horizontal(3)
	dataTX[34] = 0x00;	//??
	dataTX[35] = 0x00;	//??
	dataTX[36] = 0x00;	//??
...
// Datensendung an die Klimaanlage
...
// Aktionen mit Konfigurationsdaten

// Status des Summers abrufen
...
// Status des Klimaanlagen-Displays abrufen
...
// Modus der Fixierung der horizontalen Klappen abrufen
...
// Modus des Vertikalschwungs abrufen
...
// Modus des Horizontalschwungs abrufen
...
// Verfügbare Schwungmodi der Klappen abrufen
...
// Verfügbare Voreinstellungen abrufen
